...
```bash
# 一键生成所有密钥并写入 .env（复制整段执行即可）
AUTH_SECRET=$(openssl rand -base64 32)
JUDGE_SECRET=$(openssl rand -base64 32)
PG_PASS=$(openssl rand -base64 16)
RD_PASS=$(openssl rand -base64 16)

sed -i "s|^AUTH_CODE_SECRET=.*|AUTH_CODE_SECRET=\"${AUTH_SECRET}\"|" .env
sed -i "s|^JUDGE_CALLBACK_SECRET=.*|JUDGE_CALLBACK_SECRET=\"${JUDGE_SECRET}\"|" .env
sed -i "s|^POSTGRES_PASSWORD=.*|POSTGRES_PASSWORD=\"${PG_PASS}\"|" .env
sed -i "s|^REDIS_PASSWORD=.*|REDIS_PASSWORD=\"${RD_PASS}\"|" .env
sed -i "s|^DATABASE_URL=.*|DATABASE_URL=\"postgresql://postgres:${PG_PASS}@127.0.0.1:5432/oj?schema=public\"|" .env
sed -i "s|^REDIS_URL=.*|REDIS_URL=\"redis://:${RD_PASS}@127.0.0.1:6379\"|" .env
```

然后按需修改其余配置项（删除引号中的占位文字，填入你的值）：
...
ALIYUN_DM_ACCOUNT_NAME="你的直邮发信地址"
```
...
# 8. 创建管理员账号（删除引号中的占位文字，填入你的邮箱）
node scripts/create-admin.mjs --email "你的管理员邮箱"
...
# 申请 SSL 证书（删除引号中的占位文字，填入你的域名和邮箱）
certbot certonly --webroot -w /var/www/certbot \
  -d "你的域名" \
  --email "你的邮箱" \
  --agree-tos --no-eff-email

# 部署 Nginx 配置（同样替换为你的域名）
sed 's/YOUR_DOMAIN/"你的域名"/g' infra/nginx/codemaster.conf \
...
```bash
# 自动生成强密码（推荐，删除引号中的占位文字填入你的邮箱）
node scripts/create-admin.mjs --email "你的管理员邮箱"

# 指定密码和姓名
node scripts/create-admin.mjs --email "你的管理员邮箱" --password "你的密码" --name "你的姓名"

# 用手机号创建
node scripts/create-admin.mjs --phone "你的手机号"

# 将已注册的普通用户提升为管理员（脚本自动检测并仅授予角色）
node scripts/create-admin.mjs --email "已注册用户的邮箱"
```
